...
class SongsService {
  ...
  async getAllSongs(title, performer) {
    ...
    if (title) {
      const result = await this._pool.query(`SELECT id, title, performer FROM songs WHERE LOWER(title) LIKE LOWER('%${title}%')`);

      return result.rows;
    }
    if (performer) {
      const result = await this._pool.query(`SELECT id, title, performer FROM songs WHERE LOWER(performer) LIKE LOWER('%${performer}%')`);
